...
print_port_statistics(void)
{
    ...
    rte_log(RTE_LOG_INFO, RTE_LOGTYPE_PINGPONG, "max rtt: %" PRIu64 " us  %" PRIu64 "\n", max_rtt, rte_get_tsc_hz());
        ...
        if (nb_rx)
        {
            for (i = 0; i < nb_rx; i++)
            {
                ...
                if (rte_cpu_to_be_16(*eth_type_ptr) == ETHER_TYPE_PING)
                {
                    port_statistics.rx += 1;
                    /* do pong */
                    rte_ether_addr_copy(&eth_hdr->src_addr, &pong_ether_addr);
                    rte_ether_addr_copy(&server_ether_addr, &eth_hdr->src_addr);
                    rte_ether_addr_copy(&pong_ether_addr, &eth_hdr->dst_addr);
